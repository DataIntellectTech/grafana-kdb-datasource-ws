  ...
  async query(options: DataQueryRequest<MyQuery>): Promise<DataQueryResponse> {
           ...
           var requestList = validRequestList.map(target => {
               ...
               if(!this.ws || this.ws.readyState > 1) return this.connectWS().then(connectStatus => {
                   if(connectStatus === true && nrRequests > 0) return this.sendQueries(nrRequests, requestList, nrBlankRequests, blankRefIDs, errorList).then((series: any) => { return this.buildDataFrames(series)})
                ...
                if(nrRequests > 0) return this.sendQueries(nrRequests, requestList, nrBlankRequests, blankRefIDs, errorList).then((series: any) => { return this.buildDataFrames(series)})
