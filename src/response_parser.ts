...
export default class ResponseParser {
  ...
  mapTableData(res, req) {
    ...
    let temporalFieldInc =
      typeof req[1].queryParam.temporal_field == 'string' ? true : false;
    ...
    if (req[1].queryParam.query.type == '`function') {
      for (let col = 0; col < res.payload.columns[0].length; col++) {
        ...
        if (
          '`' + res.payload.columns[0][col].text ==
          req[1].queryParam.temporal_field
        ) {
  ...
  mapGraphData(res, req) {
    ...
    for (let g = 0; g < response.payload[0].length; g++) {
      ...
      for (curCol = 2; curCol <= colKeys.length; curCol++) {
        ...
        if (
          req[1].queryParam.query.type == '`select' &&
          req[1].queryParam.column[curCol - 2][2] !== '::'
        ) {
          fieldName = req[1].queryParam.column[curCol - 2][2];
          console.log('fieldName', req[1].queryParam.column[curCol - 2][2]);
        }

        if (response.payload[0][g][grpKeys[0]].toString() == 'x') {
          targetName = fieldName;
        } else {
            console.log('targetName', response.payload[0][g][grpKeys[0]].toString() + ' - ' + fieldName)
          targetName =
            response.payload[0][g][grpKeys[0]].toString() + ' - ' + fieldName;
        }
