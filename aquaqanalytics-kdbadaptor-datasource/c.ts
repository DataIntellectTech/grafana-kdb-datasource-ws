...
export class C {
    ...
    public deserialize(x) {
        let _this = this;
        var a = x[0], pos = 8, j2p32 = Math.pow(2, 32), ub = new Uint8Array(x), sb = new Int8Array(x),
            bb = new Uint8Array(8), hb = new Int16Array(bb.buffer), ib = new Int32Array(bb.buffer),
            eb = new Float32Array(bb.buffer), fb = new Float64Array(bb.buffer);
    ...
    public serialize(x) {
        let _this = this;
        var a = 1, pos = 0, ub, bb = new Uint8Array(8), ib = new Int32Array(bb.buffer),
        ...
        function calcN(x, dt) {
            ...
            switch (t) {
                ...
                case'array': {
                    var n = 6;
                    for (var i = 0; i < x.length; i++) n += calcN(x[i], null);
                    return n;
                }
                case'symbols': {
                    var n = 6;
                    for (var i = 0; i < x.length; i++) n += calcN(x[i], 'symbol');
                    return n;
                }
                case'string':
        ...
        function w(x, dt) {
            ...
            switch (t) {
                ...
                case 'symbol': {
                    wb(-11);
                    x = _this.u8u16(x);
                    for (var i = 0; i < x.length; i++) wb(x[i]);
                    wb(0);
                }
                    break;
                case 'string':
                ...
                case 'array': {
                    ...
                    ib[0] = x.length;
                    wn(4);
                    for (var i = 0; i < x.length; i++) w(x[i], null);
                }
                    break;
                case 'symbols': {
                    wb(0);
                    ...
                    ib[0] = x.length;
                    wn(4);
                    for (var i = 0; i < x.length; i++) w(x[i], 'symbol');
                }
                    break;
            }
        }
