    ...
    return {
        ...
        execute: function() {
            KDBDatasource = (function () {
                ...
                KDBDatasource.prototype.fieldInjectVariables = function (attrib, search, replace) {
                ...
                KDBDatasource.prototype.newGetVariables = function (templatesrv) {
                    ...
                    for (var i = 0; i < this.templateSrv.variables.length; i++) {
                        ...
                        if (templatesrv.variables[i].options[0].text === 'All') {
                            var valueArray = [];
                            for (var j = 1; j < this.templateSrv.variables[i].options.length; j++) {
                            ...
                            if (Array.isArray(result)) {
                                if (typeof (result[0]) === 'string') {
                                    ...
                                    if (Object.keys(result[0]).length > 1) {
                                        var errorResponse = 'Can only select single values. Attempted to return an object of key-value pairs. Unsafe query';
                                ...
                                var errorResponse = "Check Query. Syntax error with: [ " + result + " ]";
                                throw new Error(errorResponse);
                            }
                            return values;
                        }));
                ...
                KDBDatasource.prototype.metricFindQueryDefault = function (kdbRequest) {
                    var _this = this;
                ...
                KDBDatasource.prototype.metricFindQuerySym = function (kdbRequest) {
                    var _this = this;
                    // console.log('met',kdbRequest)
                    return new Promise(function (resolve, reject) {
                        resolve(_this.executeAsyncQuery(kdbRequest).then(function (result) {
